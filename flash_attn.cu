...
__global__ void flash_attention(const float *Q, const float *K, const float *V, float *O, float *m, float *l,
    ...
    extern __shared__ float _Q_i_[];     // Q_i : (B_r × d)-matrix
    extern __shared__ float _K_j_[];     // K_j : (B_c × d)-matrix
    extern __shared__ float _V_j_[];     // V_j : (B_c × d)-matrix
    extern __shared__ float _O_i_[];     // O_i : (B_r × d)-matrix
    extern __shared__ float _m_i_[];     // m_i : (B_r)-dim vector
    extern __shared__ float _l_i_[];     // l_i : (B_r)-dim vector
    extern __shared__ float _m_ij_[];    // ~m_ij : (B_r)-dim vector
    extern __shared__ float _l_ij_[];    // ~l_ij : (B_r)-dim vector
    extern __shared__ float _m_i_new_[]; // m_i_new : (B_r)-dim vector
    extern __shared__ float _l_i_new_[]; // l_i_new : (B_r)-dim vector
    extern __shared__ float _S_ij_[];    // S_ij : (B_r × B_c)-matrix
